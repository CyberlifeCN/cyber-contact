...
class contact_message_dao(singleton):

    @gen.coroutine
    def insert(self, data):
        # 执行一个查询
        sql = "INSERT INTO contact_message(_id, email, content, ctime) VALUES ('" +\
            data['_id'] + "','" +\
            data['email'] + "','" +\
            data['content'] + "','" +\
            str(data['ctime']) +\
    ...
    def count_pagination(self, idx, limit):
        sql = "SELECT count(*) FROM contact_message"
        logging.debug("sql: %r", sql)
        cur = yield Mysql_Connection_Pool().get_pool().execute(sql, ())
    ...
    def delete(self, _id):
        # 执行一个查询
        sql = "DELETE FROM contact_message " +\
            " WHERE _id='" + _id + "'"
        logging.debug("sql=[%r]", sql)
        cur = yield Mysql_Connection_Pool().get_pool().execute(sql, ())
        # cur.commit()
