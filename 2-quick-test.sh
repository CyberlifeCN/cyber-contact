...
do
	...
	ssh root@${ip} "systemctl stop cyber-contact; rpm -Uvh --replacepkgs --force /tmp/$file && rm -f /tmp/$file && echo success" || { echo "$ip failed" ; continue; }

	for svc in contact contact.swagger
	do
...
echo "-------------------------------------------------------------"
{
	...
	for ip in $IP_LIST
	do
		status=$(ssh root@${ip} "rpm -q cyber-contact")
		echo "$ip $status"
	done
} | column -t
